...
const versionData: Version[] = [
  ...platforms.map((p) => ({
    ...
    size: p.id === 'windows' ? '64.2 MB' : p.id === 'linux' ? '58.7 MB' : p.id === 'macos' ? '72.1 MB' : p.id === 'android' ? '32.5 MB' : '45.8 MB',
  ...
  ...platforms.map((p) => ({
    ...
    size: p.id === 'windows' ? '62.0 MB' : p.id === 'linux' ? '56.2 MB' : p.id === 'macos' ? '70.5 MB' : p.id === 'android' ? '30.8 MB' : '44.1 MB',
